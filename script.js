...
function initializeVoices() {
  ...
  // Prioritize female voices - explicitly look for female voices
  ...
  // If no female voice found, fallback to any English or Hindi voice
  if (!selectedVoice) {
    selectedVoice = voices.find(voice => 
      voice.lang === 'en-US' || 
      voice.lang === 'en-GB' || 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ensure voices are initialized (sometimes onvoiceschanged doesn't fire)
...
function setupEventListeners() {
    ...
    if (pauseButton) {
        ...
        // Update pause button state based on speechSynthesis status
        setInterval(() => {
    ...
    // Add listen button for text-to-speech
    const listenButtons = document.querySelectorAll('.listen-button');
    if (listenButtons.length === 0) {
        // If no listen buttons exist, create them for each bot message
        addListenButtonsToExistingMessages();
    }
...
function addListenButtonsToExistingMessages() {
    const botMessages = document.querySelectorAll('.bot-message');
    botMessages.forEach(message => {
        if (!message.querySelector('.listen-button')) {
            addListenButtonToMessage(message);
        }
    });
}

function addListenButtonToMessage(messageElement) {
    // Don't add listen button to thinking indicators
    if (messageElement.classList.contains('thinking')) return;
    
    const contentDiv = messageElement.querySelector('div:first-child');
    if (!contentDiv) return;
    
    const listenButton = document.createElement('button');
    listenButton.className = 'listen-button';
    listenButton.innerHTML = '<i class="fas fa-volume-up"></i>';
    listenButton.title = 'Listen to this message';
    
    listenButton.addEventListener('click', function() {
        const textToSpeak = contentDiv.textContent;
        speakTextWithFemaleVoice(textToSpeak);
    });
...
function showWelcomeMessage() {
    ...
    // Speak the welcome message only once and if enabled
...
async function handleFileUpload(event) {
    ...
    try {
        ...
        updateLastUserMessage(`[Attached File: ${file.name}]`);
...
function getConversationContext() {
    // For simple implementation, just return last 10 messages
    return conversationHistory.slice(-10);
    
    // For more advanced context management:
    // 1. You could calculate token counts
    // 2. Include system message at the beginning
    // 3. Summarize older conversation parts
}
...
function addMessage(message, isUser) {
    ...
    if (!isUser) {
        addListenButtonToMessage(messageDiv);
    }
...
function speakTextWithFemaleVoice(text) {
    ...
    try {
        ...
        // Workaround for Chrome's bug that stops speech after 15 seconds
        if (isChromeBrowser()) {
            keepSpeechAlive();
        }
    } catch (error) {
...
// Keep speech alive on Chrome (workaround for Chrome bug)
function keepSpeechAlive() {
    const intervalId = setInterval(() => {
        ...
        // This forces Chrome to not pause after 15 seconds
        speechSynthesis.pause();
        speechSynthesis.resume();
    }, 14000);
}
